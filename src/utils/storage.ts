...
export function saveMenuType(patternIndex: number): void {
  try {
    const encrypted = encrypt({ patternIndex });
    if (encrypted) {
      safeSetItem(STORAGE_KEYS.MENU_TYPE, encrypted);
    }
  } catch (error) {
    console.error('Error saving menu type:', error);
  }
}

export function getMenuType(): number {
  try {
    const saved = safeGetItem(STORAGE_KEYS.MENU_TYPE);
    const decrypted = decrypt(saved);
    return decrypted?.patternIndex ?? 0;
  } catch (error) {
    console.error('Error getting menu type:', error);
    return 0;
  }
}
...
export function toggleFavoriteMeal(
  meal: MenuItem, 
  context?: { pattern: string; day: string; timing: '朝食' | '昼食' | '夕食' }
): boolean {
  try {
    ...
    if (existingIndex >= 0) {
      ...
      const newFavorite: SavedFavoriteMeal = {
        ...
        meal: {
          ...meal,
          timing: context?.timing || meal.timing
        },
